...
function renderGalleryArtifacts(artifacts, galleryKey) {
    ...
    const grid = document.getElementById('galleryArtifactsGrid');
    if (!section || !grid) return;

    title.textContent = `Gallery ${galleryKey} Artifacts`;
    ...
    if (!artifacts || artifacts.length === 0) {
        grid.innerHTML = `
            <div class="no-results">
                ...
                <h3>No artifacts found for this gallery</h3>
            </div>
        `;
        return;
    }
    ...
    // Pagination state
    const limitSelect = document.getElementById('gaLimit');
    const pagination = document.getElementById('gaPagination');
    const currentSpan = document.getElementById('gaCurrent');
    const totalSpan = document.getElementById('gaTotal');

    let page = 1;
    let perPage = parseInt(limitSelect?.value || '9', 10);

    function renderPage() {
        const totalPages = Math.max(1, Math.ceil(artifacts.length / perPage));
        page = Math.min(page, totalPages);

        const start = (page - 1) * perPage;
        const items = artifacts.slice(start, start + perPage);

        grid.className = 'ga-grid';
        grid.innerHTML = items.map(a => `
            <div class="ga-card" data-aos="fade-up">
                <div class="ga-img">
        ...
        if (pagination) {
            pagination.style.display = artifacts.length > perPage ? 'flex' : 'none';
    ...
    // Handlers
    if (limitSelect) {
        limitSelect.onchange = () => { perPage = parseInt(limitSelect.value, 10); page = 1; renderPage(); };
    }
    window.gaNextPage = function() { page += 1; renderPage(); };
    window.gaPrevPage = function() { page = Math.max(1, page - 1); renderPage(); };
