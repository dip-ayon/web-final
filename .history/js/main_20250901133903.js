...
function renderGalleryArtifacts(artifacts, galleryKey) {
    ...
    if (!section || !slideshow || !indicators) return;
    ...
    // Build slides
    let current = 0;
    slideshow.innerHTML = artifacts.map((a, idx) => `
        <div class="gallery-slide${idx===0 ? ' active' : ''}">
            <img src="${a.image}" alt="${a.title}" onclick="openArtifactModalFromGallery(${idx})">
        </div>
    `).join('');
    indicators.innerHTML = artifacts.map((_, idx) => `<span class="${idx===0 ? 'active' : ''}" onclick="galleryGoTo(${idx})"></span>`).join('');
    ...
    window.galleryChangeSlide = function(step){ show(current + step); };
    window.galleryGoTo = function(n){ show(n); };
    window.openArtifactModalFromGallery = function(idx){
        const a = artifacts[idx];
        if (!a) return;
        const modal = document.getElementById('artifactModal');
        const modalBody = document.getElementById('artifactModalBody');
        const images = a.images && a.images.length ? a.images : [a.image];
        const imagesHtml = images.map(img => `<img src="${img}" alt="${a.title}" class="img-fluid mb-2">`).join('');
        modalBody.innerHTML = `
            <h2>${a.title}</h2>
            <div class="row">
                <div class="col-md-6">${imagesHtml}</div>
                <div class="col-md-6">
                    <p><strong>Collection No:</strong> ${a.collection_no || 'N/A'}</p>
                    <p><strong>Object Type:</strong> ${a.object_type || 'Artifact'}</p>
...
function getDummyGalleryArtifacts(galleryKey) {
    // Map of simple dummy artifacts per gallery/location id
    const base = [
        {
            title: 'Freedom Fighter Photograph',
            description: 'Portrait of a Mukti Bahini fighter during the Liberation War.',
            image: 'assets/images/img1.jpg',
            object_type: 'Photograph',
            collection_no: 'LWM-P-1971-001'
        },
        {
            title: 'Field Diary Pages',
            description: 'Handwritten diary documenting operations and daily hardships in 1971.',
            image: 'assets/images/img2.jpg',
            object_type: 'Document',
            collection_no: 'LWM-D-1971-014'
        },
        {
            title: 'Makeshift Radio Transmitter',
            description: 'Improvised transmitter used for communication across sectors.',
            image: 'assets/images/img3.jpg',
            object_type: 'Equipment',
            collection_no: 'LWM-E-1971-009'
        }
    ];

    const extra = [
        {
            title: 'Liberation War Poster',
            description: 'Propaganda poster encouraging resistance and unity.',
            image: 'assets/images/img4.jpg',
            object_type: 'Poster',
            collection_no: 'LWM-PST-1971-022'
        },
        {
            title: 'Personal Letter Home',
            description: 'Letter from the frontlines to family in Dhaka.',
            image: 'assets/images/img5.jpg',
            object_type: 'Letter',
            collection_no: 'LWM-L-1971-037'
        }
    ];

    // Vary content slightly by key
    const keyNum = parseInt(galleryKey, 10);
    if (!isNaN(keyNum) && keyNum % 2 === 0) {
...
function getGalleryMeta(galleryKey) {
    const keyNum = parseInt(galleryKey, 10) || 1;
    if (keyNum === 1) {
        return {
            title: 'Archaeology — Archaeology Collections',
            description: 'After the Indus Valley Civilization, India came into the age of different rulers and dynasties... Religious structures, forts, mausoleums, and sculptures flourished; selected examples are exhibited across the Maurya, Sunga and Satavahana, Kushan and Ikshvaku, Gupta, and Medieval art galleries.'
        };
    }
    return {
        title: `Gallery ${galleryKey} Artifacts`,
        description: 'Selected artifacts and memorabilia related to the Liberation War of Bangladesh.'
    };
}
